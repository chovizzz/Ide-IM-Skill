# CLAUDE.md — Project Guidelines for Ide-IM-Skill
...
>
> ```
> 请帮我排查 ide-im 桥接服务的问题。
> 1. 读取 $CTI_HOME/logs/bridge.log 最近 50 行（默认 ~/.claude-to-im）
> 2. 读取 $CTI_HOME/config.env 检查配置是否正确
> 3. 运行 bash .cursor/skills/ide-im/scripts/doctor.sh（或工作区内该 skill 的 scripts/doctor.sh） 并分析输出
> 4. 根据日志和配置给出具体的修复建议
> ```

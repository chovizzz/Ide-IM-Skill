...
All credentials are stored in `$CTI_HOME/config.env` (default `~/.claude-to-im/config.env`) with file permissions set to `600` (owner read/write only). This file is created during `setup` and never committed to version control.
...
3. Run `ide-im reconfigure` to update the stored credentials
4. Run `ide-im stop` then `ide-im start` to apply changes
...
2. Run `ide-im stop` to halt the daemon
3. Run `ide-im reconfigure` with a new token
4. Review `$CTI_HOME/logs/` (CTI_HOME 默认 ~/.claude-to-im) for unauthorized activity
5. Run `ide-im start` with the new credentials

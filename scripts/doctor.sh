...
# --- Cursor CLI available (cursor mode only) ---
if [ "$CTI_RUNTIME" = "cursor" ]; then
  CURSOR_AGENT=""
  if [ -n "$CTI_CURSOR_AGENT_EXECUTABLE" ]; then
    [ -x "$CTI_CURSOR_AGENT_EXECUTABLE" ] && CURSOR_AGENT="$CTI_CURSOR_AGENT_EXECUTABLE"
  fi
  if [ -z "$CURSOR_AGENT" ]; then
    for cmd in agent cursor; do
      CURSOR_AGENT=$(which "$cmd" 2>/dev/null | head -1)
      [ -n "$CURSOR_AGENT" ] && break
    done
  fi
  if [ -z "$CURSOR_AGENT" ]; then
    for cand in "$HOME/.cursor/bin/agent" "$HOME/.local/bin/agent" "/usr/local/bin/agent" "/opt/homebrew/bin/agent"; do
      [ -x "$cand" ] && CURSOR_AGENT="$cand" && break
    done
  fi
  if [ -n "$CURSOR_AGENT" ]; then
    check "Cursor CLI (agent) available ($CURSOR_AGENT)" 0
  else
    check "Cursor CLI (agent) available (install: curl https://cursor.com/install -fsS | bash)" 1
  fi
fi
...
if [ "$CTI_RUNTIME" = "claude" ] || [ "$CTI_RUNTIME" = "auto" ]; then
  ...
  if [ "$HAS_ANTHROPIC_CONFIG" = "true" ]; then
    ...
    PLIST_FILE="$HOME/Library/LaunchAgents/com.ide-im-skill.bridge.plist"
...
# --- Codex checks (codex/auto only; cursor uses Claude only) ---
